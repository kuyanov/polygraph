...
TEST(Execution, Stdio) {
    ...
    auto response1 =
        SendTask({.argv = {"bash", "-c", "./script <input >output"}, .max_threads = 3});
...
TEST(Execution, Permissions) {
    InitContainer();
    AddFile("input", "", 3);
    auto response1 = SendTask({.argv = {"bash", "-c", "cat <input"}, .max_threads = 2});
    ...
    AddFile("script", "#!/bin/bash\ncat", 6);
    auto response3 = SendTask({.argv = {"bash", "-c", "./script"}, .max_threads = 3});
...
TEST(Execution, TimeLimit) {
    InitContainer();
    auto response =
        SendTask({.argv = {"bash", "-c", "while true; do :; done"}, .time_limit_ms = 1000});
    ASSERT_TRUE(response.result.has_value());
...
TEST(Execution, WallTimeLimit) {
    InitContainer();
    auto response = SendTask({.argv = {"sleep", "2"}, .wall_time_limit_ms = 1000});
    ASSERT_TRUE(response.result.has_value());
